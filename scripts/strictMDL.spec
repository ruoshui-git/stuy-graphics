...
Items enclosed in quotes means they appear as exactly specified.
Items not enclosed in quotes ("") have definitions of formats that they should match to.

For example,
	degrees := DOUBLE
means that the placeholder (token) "degrees" is in the format of a "DOUBLE"

For example, rotate is specified as:

"rotate" x|y|z degress [knob]
	where 
			x, y, z, degrees := DOUBLE
			knob := STRING
	and 
...
DEF sections describe how some tokes are defined.
...
- which means, "-9", "95", "-0.", "-0.5", "1.", "1.5", "-.5", ".430" are all valid DOUBLE

SYMBOL := 
	[a-zA-Z][a-zA-Z0-9_]*

- i.e. SYMBOL starts with a letter (upper or lower), then includes zero or more letters (upper or lower), 
	digits, or underscore ("_") characters
- this is defined in the provided parser source code, but never actually used

STRING :=
	[a-zA-Z][\.a-zA-Z0-9_]*

- i.e. same as SYMBOL except for characters after the first one may also include dot (".") characters
...
"mesh" [constants] ":"filename [coord_system]
			...
			- `":"filename` is a token ":" followed by filename, with no space in between

DEF
	x, y, z, r, r0, r1, h, w, d := DOUBLE
	x0, y0, z0, x1, y1, z1 := DOUBLE
	constants, coord_system, coord_system0, coord_system1 := STRING
	filename := STRING
...
---
DEF
	name, knobname, knoblist, knoblist0, knoblist1, knob := STRING
	value, start_frame, end_frame, num_frames, start_val, end_val := DOUBLE

- "start_frame", "end_frame", "num_frames" are defined as DOUBLE, but really, frames should be non-negative integers
...
DEF
	r, g, b, x, y, z := DOUBLE
	name := STRING
	kar, kdr, ksr, kag, kdg, ksg, kab, kdb, ksb := DOUBLE
